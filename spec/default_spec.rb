...
describe 'openstack-network' do
  describe 'ubuntu' do
    ...
    describe '/etc/neutron/neutron.conf' do
      ...
      context 'lbaas & fwaas enabled' do
        cached(:chef_run) do
          node.override['openstack']['network_lbaas']['enabled'] = true
          node.override['openstack']['network_fwaas']['enabled'] = true
          runner.converge(described_recipe)
        ...
        [
          /^service_plugins = firewall_v2,router,neutron_lbaas.services.loadbalancer.plugin.LoadBalancerPluginv2$/,
        ].each do |line|
          it do
            expect(chef_run).to render_config_file(file.name).with_section_content('DEFAULT', line)
          end
