...
execute 'migrate fwaas database' do
  timeout timeout
  command <<-EOF.gsub(/^ {4}/, '')
    neutron-db-manage --subproject neutron-fwaas --config-file /etc/neutron/neutron.conf --config-file #{plugin_config_file} upgrade head
  EOF
